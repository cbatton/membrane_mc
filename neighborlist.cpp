...
NeighborList::NeighborList(MembraneMC* sys_) {
    // Constructor
...
void NeighborList::GenerateNeighborList() {    
	...
	for(int i=0; i<sys->vertices; i++) {
		// Determine index
		int index_x = int(sys->lengths[0]*(radii_tri[i][0]+0.5)/box_x);
		int index_y = int(sys->lengths[1]*(radii_tri[i][1]+0.5)/box_y);
		int index_z = int((radii_tri[i][2]+sys->lengths[2])/box_z);
        if(index_x == nl_x) {
            index_x -= 1;
        }
        if(index_y == nl_y) {
...
void NeighborList::GenerateCheckerboard() {    
	...
	for(int i=0; i<sys->vertices; i++) {
		// Determine index
		int index_x = floor((sys->lengths[0]*(radii_tri[i][0]+0.5)-cell_center_x)/box_x_checkerboard);
		int index_y = floor((sys->lengths[1]*(radii_tri[i][1]+0.5)-cell_center_y)/box_y_checkerboard);
        if(index_x == -1) {
